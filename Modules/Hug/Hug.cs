...
namespace SuccBot.Modules
{
    public class Hug : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task HugAsync(SocketGuildUser user)
        {
            ...
            await Context.Channel.SendFileAsync(stream, "hugged.png");
        }

        [Command("gorilla", RunMode = RunMode.Async)]
        public async Task GorillaAsync(SocketGuildUser user)
        {
            Stream stream = null;
            stream = await PictureService.EditedImageAsync(user, "gorilla", 240, 450);
            await Context.Channel.SendFileAsync(stream, "gorilla.png");
        }
        // {
